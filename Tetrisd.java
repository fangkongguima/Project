...
public class Tetrisd extends JFrame implements KeyListener {
...
//    }
    ...
    public void changeData(int m,int n){
        int temp = 0x8000;
        Color t = Color.red;
        if (rect == 0x0066) t = Color.red;
        else if (rect == 0x4444||rect==0x0f00)  t = Color.cyan;
    ...
    public void load_panel() throws FileNotFoundException {
        ...
        while(input.hasNext()){
            if (i < gameRow){
                ...
                if (t == 1) rect = input.nextInt();//检测这里是否是16进制
                if (t == 2){
                    x = input.nextInt();
                    y = input.nextInt();
                }
                if (t == 3) score = input.nextInt();
